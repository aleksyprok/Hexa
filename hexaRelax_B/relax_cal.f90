...
CONTAINS
  ...
  SUBROUTINE relax_routine
    ...
    ! DO WHILE (n .LE. 100)
      ...
      localmin = MINVAL( (/ delx / MAXVAL( (/ ABS(vx), TINY(0.0_num) /) ), &
                            dely / MAXVAL( (/ ABS(vx), TINY(0.0_num) /) ), &
                            delz / MAXVAL( (/ ABS(vx), TINY(0.0_num) /) ), &
                            delx * delx / etad /) )
  ...
  SUBROUTINE boundary_conditions(t)
    ...
    bbx(:, :, 0) = bbx_p + (2. * bbx_fix - bbx(:, :, 1) - bbx_p) * ramp_up(0.1_num * t)
    bby(:, :, 0) = bby_p + (2. * bby_fix - bby(:, :, 1) - bby_p) * ramp_up(0.1_num * t)
